...
void UI::SetEditText(size_t id, wstring text)
{
	...
	for (size_t i = 0; i < RIds.size(); i++)
	{
		if (RIds[i].id == id)
		{
			...
			if (dWriteFactory)
			{
				dWriteFactory->CreateTextLayout(edits[RIds[i].pointId].text.c_str(), (UINT32)edits[RIds[i].pointId].text.size(), editTextFormat, 0.5f, 0.5f, &edits[RIds[i].pointId].textLayout);
			}
		}
	}
}
