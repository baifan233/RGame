...
struct Page
{
	float bitmapOpacity = 1.0f;
	ID2D1Bitmap* bitmap=nullptr;
	vector<ButtonStruct*> buttonsAddr{0};
	vector<SlipStruct*> slipsAddr{0};
	vector<EditStruct*> editsAddr{0};
...
protected:
	...
	vector<ActionStruct> mouseOn{0};
	vector<ActionStruct> onClick{0};
	vector<ActionStruct> normal{0};
	vector<ActionStruct> clickUp{0};
	vector<FLOAT4Struct> float4s{0};
	int transformTime = 0;
	float opacity = 0.0f;
	D2D1_ROUNDED_RECT rect = { 0 };
	vector<TextStruct>mtexts{0};
...
public:
	...
	virtual void Draw(UI* ui)
	{
		...
		context->DrawText(text.c_str(), (UINT32)text.size(), ui->m_pText, rect.rect, ui->p_bControlsBrush);
		...
		if (f1 != -1)
		{
			...
			for (size_t i = 0; i < datas.size(); i++)
			{
				...
				if (roundX == -1.0f)
				{
					...
					{
						f2 = datas[i].find("=", f1);
						roundX = (float)atof(datas[i].substr(f2 + 1, f2 - f1 - 1).c_str());
					}
				}
				if (roundY == -1.0f)
				{
					...
					{
						f2 = datas[i].find("=", f1);
						roundY = (float)atof(datas[i].substr(f2 + 1, f2 - f1 - 1).c_str());
					}
				}
				...
				{
					f2 = datas[i].find("=", f1);
					rb->SetOpacity((float)atof(datas[i].substr(f2 + 1, f2 - f1 - 1).c_str()));
				}
