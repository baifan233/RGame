...
GDevices::~GDevices()
{
	...
	//ReleaseCom(g_p_BlackBrush);
	//ReleaseCom(g_p_bControlsBrush);
...
bool GDevices::g_InitD2D()
{
	...
	/*if (FAILED(g_dWriteFactory->CreateTextFormat(L"微软雅黑", 0, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL,
		DWRITE_FONT_STRETCH_EXTRA_CONDENSED, 30, L"", &g_m_pText)))
		return false;
	g_m_pText->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_CENTER);
	g_m_pText->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_CENTER);*/
	...
	//g_d2dContext->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White), &g_p_bControlsBrush);

	//g_d2dContext->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White), &g_p_BlackBrush);
