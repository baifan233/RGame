...
void build1(World* world, GDevices* devices, ButtonStruct* bs)
{
	...
	for (size_t i = 0; i < 100; i++)
	{
		...
		obj.AddShapeAsBox(pos.x - (double)20 * (double)i, pos.y - 200, 20, 20, world->GetSpace(), 0.7f, (int)world->GetBodySize(), RObjectMaterial::WOOD);
...
void Game::Work()
{
	...
	{
		devices->g_GetD2DRen()->SetTarget(0);
		devices->g_GetSwapChain()->Release();
		
	}
	for (size_t i = 0; i < world->GetBodys().size(); i++)
	{
		world->GetBodys()[i].rePos(devices->GetCameraRect());
	}
...
void Game::Draw()
{
	...
	if (world->bBuildMode())
	{
		...
		{
			cpVect pos2 = ScreenToSpace(MouseX, MouseY, devices->GetCameraRect(),1.0f);
			RObject obj;
			obj.AddShapeAsBox(pos2.x, pos2.y, 40, 40, world->GetSpace(), 0.7f, (int)world->GetBodySize(), RObjectMaterial::WOOD);
			world->AddObject(obj);
		}
		else if (mouseState.rgbButtons[1] == RSKeyUp)
		{
			...
			if (cpSpacePointQueryNearest(world->GetSpace(), ScreenToSpace((double)MouseX, (double)MouseY, devices->GetCameraRect(), scaleSize), 0, { CP_NO_GROUP,(cpBitmask)(1 << 31), (cpBitmask)(1 << 31) }, &info)) {
				...
				cpFloat cell_size = cpfmax(bb.r - bb.l, bb.t - bb.b) / 2.0f;   //    除以10.0f将会让cell更细
				if (cell_size > 5.0f) {
	...
	{
		if (mouseState.rgbButtons[2]==RSKeyUp)
		{
			cpVect pos2 = ScreenToSpace(MouseX,MouseY,devices->GetCameraRect(), 1.0f);;
			//lights.push_back(new Light({ (float)pos2.x,(float)pos2.y }, 4000.0f, 0.6f,1.0f,1.0f,1.0f));
		}
		if (mouseState.rgbButtons[1] == RSKeyUp)
		{
			cpVect pos2 = ScreenToSpace(MouseX, MouseY, devices->GetCameraRect(), 1.0f);;
			//lights.push_back(new Light({ (float)pos2.x,(float)pos2.y }, 400.0f, 0.6f));
		}
	}
	...
	if (d2dContext)
	{
		...
		//wstring text = wtemp;
		//d2dContext->DrawText(text.c_str(), (UINT32)text.size(), resourceManager.GetTextFormat(), { 800, 100, 900,200 }, resourceManager.GetTRS(0)->brush);
		...
		//	text = wtemp;
		//	d2dContext->DrawText(text.c_str(), (UINT32)text.size(), resourceManager.GetTextFormat(), { 200, 200, 400,300 }, resourceManager.GetTRS(0)->brush);
		//}
		//swprintf_s(wtemp, L"WASD移动 Q缩小 E放大  TAB设置");
		//text = wtemp;
		//d2dContext->DrawText(text.c_str(), (UINT32)text.size(), resourceManager.GetTextFormat(), { 0,0, 400,300 }, resourceManager.GetTRS(0)->brush);
