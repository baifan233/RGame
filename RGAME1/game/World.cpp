...
void World::Init(GDevices* gdevices, ResourceManager* rm)
{
	...
	obj.AddShapeAsBox(0, 1000, width, 800, space, 1.0, GetUsableId(), RObjectMaterial::BASELAND);
...
void World::Draw(RObjectType objType, float scaleSize)
{
	...
	//ID2D1BitmapBrush* brush = resourceManager->GetTexture("stone").brush;
	...
	////brush->SetTransform(D2D1::Matrix3x2F::Translation(10,10));
	...
	//				{
	//					ptemp = cpPolyShapeGetVert(bodys[i].GetShape(), (int)j);
	//					float tempa = 0.9f;
	...
	//			break;
	//		case cpShapeType::CP_NUM_SHAPES:
	//			break;
	//		default:
	//			break;
	//		}
