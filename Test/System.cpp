...
bool System::InitD2D()
{
	...
	if (!this->d2dHwndRenderTarget)//如果还没初始化   (防止重复初始化)
	{
		if (mainWindow)//如果窗口句柄不为空就创建窗口句柄的
		{
			...
			//dc3->DrawSpriteBatch(spb,g_bitmap);
			return true;
		}
		else
			return false;
...
void System::D2DRender(bool Present, const wchar_t* text)
{
	if (d2dRenderTarget)
	{
		...
		int dstrlen = lstrlenW(text);
		if (dstrlen > 0)
			d2dRenderTarget->DrawText(text, dstrlen, m_pText, { 0, 0, (float)width/2, (float)height/2}, p_BlackBrush);
		if (Present)
		{
	...
	if (d2dHwndRenderTarget)
	{
		...
		int dstrlen = lstrlenW(text);
		if (dstrlen > 0)
			d2dHwndRenderTarget->DrawText(text, dstrlen, m_pText, {0,0, (float)width / 2, (float)height / 2 }, p_BlackBrush);
