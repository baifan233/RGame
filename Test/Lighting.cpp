...
void Light::draw(std::vector<m_Polygon*>*scene) {
	...
	glBegin(GL_TRIANGLE_FAN);
		...
		for(int i=0; i <= NUM_POINTS; i++) {
			float angle = i * 2 * 3.14159265f / NUM_POINTS;
			glVertex3f(pos.x + (cos(angle) * radius), pos.y + (sin(angle) * radius), outerDepth);
		}
	glEnd();
}

void Light::drawAlpha(std::vector<m_Polygon*>*scene) {
	glBegin(GL_TRIANGLE_FAN);
...
void Light::drawShadows(std::vector<m_Polygon*> *scene) {
	for(int i=0; i < scene->size(); i++) {
		std::vector<m_Edge*> away = scene[0][i]->getNotFacingEdges(pos);
